# read embargo area
# extract polygons
# if no poligon is provided or data is corrupted see if it contains a coordinate
# if coordinats are provided, intersect with properties of car
# add property polygon to embargoed area
# 
# two columnes: embargo start year, embargo end year
...
### find intersecting properties for MT, PA, RO
...
# intersect
MT_car_embargoed <- sf::st_intersection ( MT_car %>% select(id),  MT_emb_2intersect_latlong_sf) 
MT_car_embargoed_poly <- MT_car %>% select(id) %>% inner_join(as_tibble (MT_car_embargoed )%>% select(-geometry), by="id")# keep polygons, not points!
...
# intersect
PA_car_embargoed <- st_intersection (PA_car, PA_emb_2intersect_latlong_sf) 
PA_car_embargoed_poly <- PA_car %>% select(id) %>% inner_join(as_tibble (PA_car_embargoed )%>% select(-geometry), by="id")
...
# intersect
RO_car_embargoed <- st_intersection (RO_car, RO_emb_2intersect_latlong_sf) 
RO_car_embargoed_poly <- RO_car %>% select(id) %>% inner_join(as_tibble (RO_car_embargoed )%>% select(-geometry), by="id")
...
########################
# add embargo area
CAR_embargoe_all_data <- CAR_embargoe_all %>% left_join (emb_row, by="row") 
